...
impl<'a> LayoutBox<'a> {
    ...
    fn calculate_text_size(&mut self) -> Value {
        ...
        match self.box_type {
            BoxType::BlockNode(style_node) | BoxType::InlineNode(style_node) => {
                match style_node.node.node_type {
                    NodeType::Text(ref s) => {
                        let font_size = if let Some(size) = style_node.value("font-size") {
                            match size {
                                Length(size, Px) => size,
                                _ => unimplemented!(),
                            }
                        ...
                        return Length(font_size * s.len() as f32, Px);
                    }
                    _ => {}
                }
            }
    ...
    fn calculate_block_width(&mut self, containing_block: Dimensions) {
        ...
        if width == auto {
            width = self.calculate_text_size();
        }
    ...
    fn calculate_block_height(&mut self) {
        if let Some(Length(h, _px)) = self.get_style_node().value("height") {
